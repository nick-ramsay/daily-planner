...
const Home = () => {
  ...
  const renderPlans = () => {
    API.findAllPlans(getCookie("account_id")).then((res) => {
      for (let i = 0; res.data.length > i; i++) {
        if (
          moment(res.data[i].created_date).format("MM/DD/YYYY") ===
  ...
  const createPlan = (event) => {
    API.createPlan(getCookie("account_id"), "Open", new Date()).then((res) => {
      renderPlans();
    });
  };
        ...
        <div className="pb-2 my-5 mb-4 px-5">
          <div className="col-md-12">
            ...
            {!loading === true &&
              Plans.map((plan, i) => (
                <div
                  className="container mt-2 mb-2 plan-card"
                  onLoad={datadogRum.addTiming("task-card" + i)}
                  key={i}
                >
                  <div className="pt-1">
                    <div className="mt-1 mb-1 text-center">
                      <strong>
                        <h4>
                          {moment(plan.created_date).format(
                            "dddd,  DD MMMM YYYY"
                          )}
                        </h4>
                      </strong>
                    ...
                    {(() => {
                      ...
                      for (let j = 0; j < plan.tasks.length; j++) {
                        if (plan.tasks[j].status === "Closed") {
                          ...
                          plan.tasks[j].status === "Pending Feedback"
                        ) {
                          pendingFeedbackTaskCount += 1;
                        } else if (
                          plan.tasks[j].status === "Awaiting Backport"
                        ) {
                      ...
                      if (openTaskCount > 0) {
                          ...
                          <div>
                            <h6>
                              <span className="badge badge-danger">
                                {openTaskCount}{" "}
                                {openTaskCount > 1 ? "tasks" : "task"} open
                              </span>
                            </h6>
                          </div>
                        );
                        ...
                        return (
                          <div>
                            ...
                            <p style={{ fontSize: 14 }}>
                              {puntedTaskCount}{" "}
                              {puntedTaskCount === 1 ? "task" : "tasks"} punted
                              {pendingFeedbackTaskCount > 0
                                ? ", " +
                                  pendingFeedbackTaskCount +
                                  " pending feedback"
                                : ""}
                              {awaitingBackportTaskCount > 0
                                ? ", " +
                                  awaitingBackportTaskCount +
                                  " awaiting backport"
                                : ""}
                            </p>
                          </div>
                    ...
                    <div>
                      <a
                        className="btn btn-sm btn-custom mt-1 mb-2"
                        href={"./plan/" + plan._id}
                      >
                        View Tasks
                      </a>
